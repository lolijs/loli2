...
var less = require('/usr/local/share/npm/lib/node_modules/less');
...
function run(dir, force) {
	var cfgName = {
		...
		'amd': 'external/require/require.config.js'
	...
	function buildLess() {
		...
		}).parse(fs.readFileSync(orgfile, {
			encoding: 'utf8'
		}), function(err, tree) {
			if (err) {
				console.log('error occurs while compiling ' + orgfile + '\n' + err);
			} else {
				fs.writeFileSync(distfile.replace(/less$/, 'css'), tree.toCSS({
					compress: true
				}));
				ln++;
				checkFiles();
